...
const CommentItem: React.FC<CommentItemProps> = ({ comment, unitId, canDelete }) => {
  ...
  return (
    <div className="flex gap-2 py-2.5 border-b last:border-b-0">
      <div className="w-7 h-7 flex-shrink-0 rounded-full bg-primary-100 flex items-center justify-center">
        <span className="text-xs font-semibold text-primary-600">{initial}</span>
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          <span className="text-xs font-semibold text-gray-800">{comment.authorName ?? '익명'}</span>
          {isNew && (
            <span className="px-1 py-0 text-[10px] font-bold bg-red-500 text-white rounded">NEW</span>
          )}
...
export const UnitCommentSection: React.FC<UnitCommentSectionProps> = ({ unitId }) => {
  ...
  const handleSubmit = async () => {
    ...
    await addComment.mutateAsync({ unitId, content: trimContent })
    setContent('')
  }

  const canDeleteComment = (c: UnitComment) =>
    !!me && (
      c.authorId === me.id
      || me.role === 'BUILDING_OWNER'
      || me.role === 'ADMIN'
    )
    ...
    <div className="border-t mt-1">
      ...
      {me ? (
        <div className="px-5 py-3 bg-gray-50 border-t space-y-2">
          ...
          <p className="text-[10px] text-gray-400">Ctrl+Enter로 전송 — {me.name ?? me.email} 으로 작성됩니다</p>
        </div>
      ) : null}
    </div>
  )
