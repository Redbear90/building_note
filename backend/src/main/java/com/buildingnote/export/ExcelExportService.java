...
/**
 * 현재 사용자 워크스페이스 데이터를 엑셀로 내보낸다.
 * 활성(미삭제) 기록만 포함하며, 슬라이드 ON 여부는 unit.is_active 컬럼 기준.
 */
@Service
...
public class ExcelExportService {
    ...
    public byte[] exportMyOrganization() throws IOException {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ...
            List<Zone> zones = zoneRepository.findByOrganizationIdOrderByCreatedAtDesc(orgId);
            List<Building> allBuildings = buildingRepository.findAllInOrg(orgId);
            ...
            List<UUID> allUnitIds = unitsByBuilding.values().stream()
                    .flatMap(List::stream).map(Unit::getId).toList();

            // 멤버별 1건이므로 unitId → List<UnitRecord> 형태
            Map<UUID, List<UnitRecord>> recordsByUnit = unitRecordRepository
                    .findActiveByUnitIdIn(allUnitIds)
                    .stream()
            ...
            for (Map.Entry<String, List<Building>> entry : byZone.entrySet()) {
                ...
                for (Building building : buildings) {
                    ...
                    buildingCell.setCellValue("▶ " + building.getName()
                            + (building.getAddress() != null ? " (" + building.getAddress() + ")" : ""));
                    ...
                    createHeaderCell(colHeader, col++, "작성자", headerStyle);
                    ...
                    for (Unit unit : units) {
                        List<UnitRecord> recs = recordsByUnit.getOrDefault(unit.getId(), List.of());
                        if (recs.isEmpty()) {
                            Row dataRow = sheet.createRow(rowIdx++);
                            writeUnitRow(dataRow, unit, null, sortedFields, wrapStyle);
                        } else {
                            for (UnitRecord rec : recs) {
                                Row dataRow = sheet.createRow(rowIdx++);
                                writeUnitRow(dataRow, unit, rec, sortedFields, wrapStyle);
                            }
                        }
                    }
                    rowIdx++;
    ...
    private void writeUnitRow(Row dataRow, Unit unit, UnitRecord record,
                              List<FormField> sortedFields, CellStyle wrapStyle) {
        ...
        dataRow.createCell(col++).setCellValue(Boolean.TRUE.equals(unit.getIsActive()) ? "ON" : "OFF");
        dataRow.createCell(col++).setCellValue(record != null ? record.getAuthor().getName() : "");
